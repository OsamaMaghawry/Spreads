...
- `docs/product-context.md` is generated by `npm run context` from the screens, edge functions, schema and analytics in the repo. It is the single current description of the product — hand it to a model or a reviewer instead of explaining from memory. Never edit it by hand; CI fails when it no longer matches the code (`npm run context:check`).
