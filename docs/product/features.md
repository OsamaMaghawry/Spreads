...
**Correction, 2026-09-07 (end-of-day pass):** S3 and T1 were still marked
`staging` here. Both shipped to `main` on 2 Sep (`f8c427b`, `acc57a4`) —
`git log origin/main -- supabase/functions/_shared/optionScan.ts` shows
`SINGLE_STRATEGIES`/`buildSingle` on `main`, and `openPosition/index.ts` and
`StrategyPicker.jsx` on `main` both carry the single-leg and wheel path. This
table had been stale for five days; fixed below rather than left to mislead
the next reader.
