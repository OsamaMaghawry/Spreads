...
| **Ours** | | | |
| dashboard.deltamint.app / deltamint.app | ❌ 403 (policy, at CONNECT) | 2026-09-16 | Was ✅ 200 on 2026-08-31; blocked since 31 Aug per `docs/ops/queue.md`'s 2026-09-02 allowlist item (already open, not re-escalated here). Re-tested 04 Sep: `/`, `/blog`, `/pricing` and `dashboard.deltamint.app` all 403 at the proxy CONNECT, same failure mode as the other allowlist rows above — not a site-side issue. Re-tested 16 Sep for the seo-editor site check (`/blog`, `/sitemap.xml`, `/blog/feed.xml` via curl, `/blog` again via WebFetch): identical 403 at CONNECT / `EGRESS_BLOCKED` every time. Still open, not re-escalated |
...
| yecfbeohyakuoyczvdbj.supabase.co (production project, edge functions incl. `sendDigest`) | ❌ 403 (policy, at CONNECT) | 2026-09-16 | duty-engineer tried to email the owner via `sendDigest` per its brief (`.claude/agents/duty-engineer.md`) over a live production security finding and got `CONNECT tunnel failed, response 403` — same failure mode as `deltamint.app`. Not tested before; needs an allowlist addition alongside the `dashboard.deltamint.app` item already open in `docs/ops/queue.md` if the duty-engineer email step is meant to work from this environment. Worked around by using `PushNotification` to reach the owner directly instead — recorded, not routed around. Re-tested 16 Sep for the weekly seo-editor growth report's email step: identical `CONNECT tunnel failed, response 403`. Worked around the same way — `PushNotification`, not a routed-around request |
