...
| **Ours** | | | |
| dashboard.deltamint.app / deltamint.app | ❌ 403 (policy, at CONNECT) | 2026-09-04, re-tested 2026-09-23 | Was ✅ 200 on 2026-08-31; blocked since 31 Aug per `docs/ops/queue.md`'s 2026-09-02 allowlist item (already open, not re-escalated here). Re-tested 04 Sep: `/`, `/blog`, `/pricing` and `dashboard.deltamint.app` all 403 at the proxy CONNECT, same failure mode as the other allowlist rows above — not a site-side issue. Re-tested 23 Sep for seo-editor's site check (`/blog`, `/blog/foundations`, a post, `/sitemap.xml`, `/blog/feed.xml`, via both WebFetch and `curl`): unchanged, still 403 at CONNECT on every path. Structured-data check against the live site cannot run from this environment; not routed around |
...
| wpwaomzgpbozzghohwmf.supabase.co (staging project, edge functions) | ❌ 403 (policy, at CONNECT) | 2026-09-14 | Tested for the first time by duty-engineer, looking for any reachable `sendDigest` after the production one failed. Blocked identically. **So no Supabase host of ours is reachable from a session** — the brief's "email the owner through `sendDigest`" step cannot run at all, on either project. Also tried `email-digest.yml` (this repo's CI substitute): `workflow_dispatch` returns `403 Resource not accessible by integration` for the session token. Recorded, not routed around; filed as a `needs owner` item in `docs/ops/queue.md` |
| yecfbeohyakuoyczvdbj.supabase.co (production project, edge functions incl. `sendDigest`) | ❌ 403 (policy, at CONNECT) | 2026-09-09, re-tested 2026-09-23 | duty-engineer tried to email the owner via `sendDigest` per its brief (`.claude/agents/duty-engineer.md`) over a live production security finding and got `CONNECT tunnel failed, response 403` — same failure mode as `deltamint.app`. Not tested before; needs an allowlist addition alongside the `dashboard.deltamint.app` item already open in `docs/ops/queue.md` if the duty-engineer email step is meant to work from this environment. Worked around by using `PushNotification` to reach the owner directly instead — recorded, not routed around. Re-tested 23 Sep for the seo-editor weekly growth-report email, same command shape (`curl` with the anon key bearer token): unchanged, still `CONNECT tunnel failed, response 403`. Used the same `PushNotification` workaround, plus sent the full report to the owner as a file via `SendUserFile` since a push notification alone can't carry a one-page report |
