...
| **Ours** | | | |
| dashboard.deltamint.app / deltamint.app | ❌ 403 (policy, at CONNECT) | 2026-09-09 | Was ✅ 200 on 2026-08-31; blocked since 31 Aug per `docs/ops/queue.md`'s 2026-09-02 allowlist item (already open, not re-escalated here). Re-tested 04 Sep and again 09 Sep (seo-editor, checking `/blog`, a category hub, one post, `/sitemap.xml`, `/blog/feed.xml`): still 403 at the proxy CONNECT, same failure mode as the other allowlist rows above — not a site-side issue. Structured data itself (BlogPosting JSON-LD, canonical, OG tags, sitemap, RSS) verified by reading `landing/src/index.js` instead |
...
| yecfbeohyakuoyczvdbj.supabase.co (production, from an interactive/routine Claude session) | ❌ 403 (policy, at CONNECT) | 2026-09-09 | seo-editor's weekly task asks the agent to POST `/functions/v1/sendDigest` itself with the `.env.production` anon key — `curl` to it fails with `CONNECT tunnel failed, response 403` (`connect_rejected`), same policy-denial shape as the deltamint.app rows above. Not a Supabase-side or auth problem: `docs/ops/queue.md`'s 2026-09-07 fixed entry shows the identical function reached successfully from **GitHub Actions** (`email-digest.yml`, using `SUPABASE_SERVICE_ROLE_KEY`) two days earlier — that workflow runs outside this proxy entirely. `email-digest.yml`'s own header comment states routine sessions run without service-role credentials and cannot call `sendDigest` itself, which this confirms is also true at the network layer, not only the credentials layer. See the queue ticket for the fix (allowlist this host for sessions, or route seo-editor's growth report through the existing CI digest path) |
