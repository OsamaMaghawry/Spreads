...
| yecfbeohyakuoyczvdbj.supabase.co (production Supabase project, incl. `sendDigest`) | ❌ 403 (policy, at CONNECT) | 2026-09-08 | Same failure mode as the row above — `curl -X POST .../functions/v1/sendDigest` never leaves the gateway (`CONNECT tunnel failed, response 403`), confirmed by `docs/reality/2026-W37.md` on 2026-09-07 and re-confirmed today. This is the host the Tuesday product cadence's own email instructions target, so **the digest email cannot be sent from an agent session while this stays blocked** — same root cause as the `dashboard.deltamint.app` row, one allowlist fix (`yecfbeohyakuoyczvdbj.supabase.co`) would likely clear both the app host and this one if they share a Cloudflare/edge config. Workaround used: none available within the rules (see "the one rule that overrides" below) — reported here and via a direct notification instead of a silent failure |
