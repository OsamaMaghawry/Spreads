...
Thursday cadence, 2026-09-10 (`docs/trading-audit/2026-W37.md`). This week's added scope was the single-leg path — CSPs, covered calls, the wheel prefix, the naked-call watch rule.

- [open] 2026-09-10 · head-of-trading · **`SpreadTable.jsx`'s own "Total" footer disagrees with the account's "Max risk" tile, same screen, same sync, on a short call covered by a long call struck below it (a PMCC).** Executed: long TSLA 352.50C @13.57 / short TSLA 375C @2.26, no shares — `pairSpreads()` returns `covered_call` row `maxRisk: 0` and `long_option` row `maxRisk: 1357`. `SpreadTable.jsx:36` sums the two rows' `.maxRisk` directly → footer total $1,357. The backend's `account.totals.risk` (shown in `AccountSummaryCard`'s "Max risk" tile) reprices the ticker's whole book via `stressTotal`/`stressPL` instead → $1,131 at spot $370. $226 apart — exactly the short leg's collected premium × 100 — and it scales with size. The credit-direction shape (long strike *above* the short) does not have this bug; the naive sum happens to be algebraically correct there, which is why no existing test caught it. Fix: make `SpreadTable.jsx`'s footer read `account.totals.risk` (already available one level up in `AccountSection.jsx`) instead of re-deriving a total from `.maxRisk`, or route stock-like rows through `stressTotal` there too.
- [open] 2026-09-10 · head-of-trading · **The same covered-call row's break-even computes to −$2.26.** `positionKinds.ts`'s `breakEvenOfKind`, `COVERED_CALL` case (line 271-272), always returns `(shareBasis || 0) − credit` with no branch for `p.coverLongs` — unlike `riskOfKind`'s `COVERED_CALL` case three lines below it, which already branches on `coverLongs` for this exact shape. When the cover is a long call and no shares are held on the ticker, `shareBasis` defaults to 0, so the function returns a negative stock price instead of withholding. Same fixture as above. Fix: give `breakEvenOfKind` the same `coverLongs` branch `riskOfKind` already has, returning `null` when longs cover it and no shares exist rather than a shares-basis formula that does not apply.
- [open] 2026-09-10 · head-of-trading · **2026-W36's Finding 2 is still unfixed, one week later, and never got a queue ticket.** `syncAccounts/index.ts` computes a flat 2-leg spread's `maxRisk` (line 376) before `isAdjusted` is known (line 442) and attaches it unconditionally; `positionKinds.totalRisk` (line 382-395) only excludes a row when `maxRisk` is null, with no knowledge of `.adjusted`. Re-executed: `totalRisk([{type:"put_spread", adjusted:true, maxRisk:183}])` → `{risk:183, complete:true}` — a fabricated width-based figure on an adjusted contract still silently enters the account total with no floor indicator. Fix unchanged from last week: null out `maxRisk` on an `adjusted` flat-spread row before `totalRisk` sees it, or teach the reducer to treat `.adjusted` as undefined risk.
