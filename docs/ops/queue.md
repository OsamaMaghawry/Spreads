...
Head-of-branding's light audit of 2026-09-16 (`docs/branding/2026-W38.md`), this cycle's top five, ranked by exposure:

- [open] 2026-09-16 · head-of-branding · **Two parallel design systems — trader-facing screens ignore the `dm` palette, two cycles running.** `dm-accent` stays confined to `Admin.jsx`, `admin/*`, `AuthLayout.jsx`, `Layout.jsx`; every trading screen (`Scanner.jsx`, `Dashboard.jsx`, `Accounts.jsx`, `AccountDetail.jsx`, `AccountHistory.jsx`, `AccountAnalysis.jsx`, close/open dialogs) still runs Tailwind emerald/slate, e.g. `ExportPdfButton.jsx:286` `bg-emerald-50 border-emerald-200 text-emerald-700`. Fix: repoint primary-action styling (buttons, focus rings, active nav) at `dm-accent`; retire the ad hoc emerald/near-black buttons.
- [open] 2026-09-16 · head-of-branding · **Red means "loss/risk" and now renders as six different colours.** `dm-negative` (`#993C1D`) used once (`OAuthCallback.jsx:109`); `rose-600`/`rose-700` at 51 sites; PDF paper banner `#B4485C` (`ExportPdfButton.jsx:130`); new withheld-trade banner `rgb(160,60,60)` (`ExportPdfButton.jsx:139`); shadcn `--destructive` `#ef4444` (`src/index.css`). Fix: collapse every loss/risk/warning use to the `dm` scale, including the new PDF withheld-banner colour.
- [open] 2026-09-16 · head-of-branding · **Foundations blog posts run over the word-count rule `brand.md` was rewritten to enforce.** `content/blog/theta-decay-explained.md` (2,000 words) and `content/blog/implied-volatility-options-explained.md` (2,011 words) both published and reviewed after the "900-1,200 words, hard ceiling 1,300" rule existed; `options-bid-ask-spread.md` (1,409) and `option-delta-explained.md` (1,527) are also over. Fix: split each into two posts per the rule's own instruction, starting with theta-decay and implied-volatility.
- [open] 2026-09-16 · head-of-branding · **Green is also used decoratively, not just for gains.** The same emerald that means "gain" in P/L cells is also the scan-progress colour (`Scanner.jsx`), the connected-status dot (`AccountSection.jsx`), and every primary button in the design-system item above. Fix: reserve mint/emerald for P/L and connection-state truth only; move decorative "primary action" and "in-progress" green to `dm-accent` or a neutral.
- [open] 2026-09-16 · head-of-branding · **The exported Analysis PDF carries a non-canonical name, now on every page instead of one.** `AccountAnalysis.jsx:492` passes `` `${name} — Performance Analysis` `` as the PDF title (canonical name is "Analysis"); `ExportPdfButton.jsx:184-185` prints `"DeltaMint — economic performance report..."`, drawn via `drawFooter()` on every page since `ab47f13`, up from one page previously. Fix: change `AccountAnalysis.jsx:492` to `` `${name} — Analysis` `` and update the footer identity string to match. Hand-off: whether "economic performance report" is deliberate compliance wording is desk-editor/compliance-gate's call, not branding's.
