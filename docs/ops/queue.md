...
- [needs owner] 2026-09-21 · reality check · **PR #9 (`staging` → `main`) is no longer the "docs only" ledger PR its own description claims — it is now the whole product.** Opened 14 Sep as a docs-only ledger; because its head tracks live `staging`, its current diff is 55 files / 6,649 insertions / 780 deletions: two full broker integrations (SnapTrade, Tradier), two migrations (`0055`, `0056`), the Analysis window-accuracy fix from the digest/$2.7k incident, and the noindex site-health work (see the two items below). Still open, `mergeable_state: unknown`, zero CI statuses posted, last updated 20 Sep. Needs the owner (or agent-manager) to either merge it or re-scope it — it has silently stopped being the small thing it was opened as. Verified: `git diff --stat origin/main origin/staging`; `mcp__github__pull_request_read` on PR #9.
- [needs owner] 2026-09-21 · reality check · **Migration `0054`'s hard deadline (confirm on production before Sat 19 Sep 13:00 UTC, or the weekly digest fails to send) has passed with nothing recording whether it was met.** Set by duty-engineer 17 Sep (`docs/ops/queue.md` on `staging`). No ledger or queue entry after 18 Sep 20:06 UTC exists to say whether `0054` was applied, or whether Saturday's `weekly-digest` cron sent successfully. Needs the owner to confirm the migration and someone to check the cron's actual run outcome. Verified: reading `staging`'s queue.md entry and its dated deadline; `git log` shows no duty-engineer run after 18 Sep 20:06 UTC.
- [needs owner] 2026-09-21 · reality check · **The Vault `service_role_key` mix-up (12 Sep) is still open nine days later — every `pg_cron` trigger in the product (`trigger_trade_sync`, `trigger_position_watch`, `trigger_weekly_digest`, `trigger_equity_history`) reads a secret that actually holds the anon key on staging, and any caller with the (published) anon key can trigger `syncTrades`/`positionWatch` for all accounts.** Original entry: `docs/ops/queue.md` on `staging`, 12 Sep. Production status still unverified (no DB access from this session). This has sat past the org's own "fixed within a working hour or escalated the same run" rule for nine days. Verified: reading the staging-only queue entry; confirmed still present with no `fixed` marker as of 21 Sep.
- [needs owner] 2026-09-21 · reality check · **`main`'s copy of this very file is a week stale and tells a disproven story.** It still lists, as open, that `publish-blog.yml` fails at its credentials guard — `staging`'s copy shows this was misdiagnosed on 14 Sep (the guard passed; the real failure was one step later and every run since 13 Sep has been green) — and says nothing about the escalation channel being fully dead (sendDigest, `email-digest.yml` dispatch, and a GitHub issue were all tried and refused on 14 Sep; PR #9 was the only channel that worked). Whoever merges PR #9 should carry the corrected queue entries over. Verified: `git diff origin/main origin/staging -- docs/ops/queue.md`.
- [needs owner] 2026-09-21 · reality check · **Real Search Console data (quoted in a `staging`-only commit, `c1fddba`, 20 Sep) shows dashboard pages — including `/forgot-password` — still getting Google impressions (24 of 77 total) despite `worker/index.js` setting `X-Robots-Tag: noindex, nofollow` unconditionally in code.** A second commit the same day quotes Search Console flagging a live marketing page as "Excluded by 'noindex' tag" on 13 Sep, while `/` and `/blog` spot-checks read clean — a one-page sample was hiding a real leak. The check built to catch this (`scripts/site-health.mjs`, now walking the full sitemap and asserting the dashboard specifically) exists only on `staging` (part of PR #9) and has never run against `main` in CI. Needs: merge PR #9 (or cherry-pick the site-health script), let it run against `main`, and re-pull Search Console to confirm the leak is actually closed. Verified: reading the three `staging`-only `scripts/site-health.mjs` commits (`cb49619`, `e60cf06`, `c1fddba`) verbatim for their quoted figures; confirming `worker/index.js:40` is correct in code; confirming `site-health.yml` only triggers off `main`.
- [needs owner] 2026-09-21 · reality check · **`docs/product/pricing.md` (7 Sep) is stale, not the product — worth a one-line correction, not a fix.** It still frames `/pricing` as something to check live. In fact `95b03b7` (12 Sep) deleted `landing/public/pricing/` entirely on the owner's own direct instruction, quoted in the commit ("hide the pricing page… I don't want to have the pricing on something doesn't exist yet"), as part of turning on DEMO MODE. The implementation matches the owner's intent exactly — not drift in the product, just an unlinked planning doc. vp-product: add one line noting pricing was intentionally pulled 12 Sep. Verified: `git log --diff-filter=D -- landing/public/pricing/`; reading `95b03b7`'s full message.
- [needs owner] 2026-09-21 · reality check · **`docs/context/handoff.md` (31 Aug) is three weeks stale and actively misleading** — it lists BREVO_API_KEY/SMTP as "waiting on the owner" when circumstantial evidence (the weekly digest has sent mail since 7 Sep, plus this week's two-sender email split) suggests it's resolved, and it doesn't mention the `/forgot-password` noindex finding above, which is exactly the page its own item 3 (`wrangler deploy` for that fix) was about. Needs a rewrite or a pointer to `docs/ops/queue.md` as the live record. Verified: reading `handoff.md` against this week's queue and email-related commits.
- [needs owner] 2026-09-21 · reality check · **Third consecutive week (W36, W37, W38) where every weekly strategy Routine in `docs/context/org.md`'s rhythm table — including this job's own prior edition — produced real output that never left its own throwaway branch.** Last week: reality check, vp-growth's play, vp-product's Tuesday reconciliation, head-of-branding's audit, head-of-trading's audit (2 blocking findings), and the Friday board pack all ran on schedule with real content, on six different branches, none merged to `staging` or `main`. Unmerged `claude/*` branches: 20 two weeks ago → 49 last week → 54 today. This is the exact failure this job exists to catch, now proven structural rather than incidental — worth a process fix (an auto-PR per cadence branch, or a merge step in each Routine) rather than another week of the same finding. Verified: `git branch -a --contains <commit>` and `git merge-base --is-ancestor` for each routine's producing commit against `origin/staging`/`origin/main`; `git branch -r | grep -c claude/`.
- [needs owner] 2026-09-21 · reality check · **GA4/Search Console still not connected, 19 days after being queued (2 Sep).** Funnel is flat for the fifth straight week: 4 signups (0 new in any 7-day window), 0 paying, against a 100-paying-user target by 31 Dec — re-verified against today's own `docs/growth/metrics/2026-09-21.json`, not just the board pack's claim. Without GA4/Search Console there is no visitor count to even diagnose the flat funnel against. Verified: reading today's metrics JSON directly; cross-checked against `docs/board/2026-W38.md`'s figures for the prior week (identical).
- [needs owner] 2026-09-21 · reality check · **`sendDigest` is still 403 at CONNECT from this session — the fifth weekly routine this exact host has silently eaten mail for.** Confirmed by actually attempting the POST this job's own instructions specify, with the production anon key: `curl: (56) CONNECT tunnel failed, response 403`, no HTTP status, request never reached Supabase. Same allowlist item already open below (`yecfbeohyakuoyczvdbj.supabase.co`, 2026-09-09). Worked around this run via `PushNotification`, which this session has (not every session does). Verified: direct `curl` attempt this run, after the report and these tickets were committed.
