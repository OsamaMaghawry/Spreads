...
Head-of-trading's standing audit, 2026-W39 (`docs/trading-audit/2026-W39.md`,
against `main` `cda8571`). One ticket per finding; W36 Finding 2 is filed here
because it never reached the queue.

- [open] 2026-09-24 · head-of-trading · **A covered call written against a long call with no shares shows Break-Even −$0.80, "Max loss (stock to 0) $0.00" and a ladder rung at a negative price.** The shape the Scanner started offering this week (`3964cb6`/`95a7b3f`). `spreadPairing.ts` `coverExtra` (617–630) defaults `shareBasis` to 0 when no share lot exists; `breakEvenOfKind(COVERED_CALL)` returns `0 − credit`. Executed: long IBIT 261218 40C @6.00 + short IBIT 261016 45C @0.80 → `covered_call maxRisk 0 breakEven -0.8 shareBasis 0`. The ticket withheld this break-even (`buildCallOverLong`: null); the dashboard invents it. Blocking-class default. Plain fix: no 0 default; break-even null and no "(stock to 0)" label when cover is by longs only. A new kind for the shape changes meaning, so that part goes to the founder.
- [open] 2026-09-24 · head-of-trading · **`syncAccounts` re-defaults the break-even `positionKinds` withholds on an adjusted short call, and `StrikeLadder` draws and colours it.** `syncAccounts/index.ts:304` `s.breakEven ?? (strike ± premium)` overrides `breakEvenOfKind(SHORT_CALL_UNJUDGED) === null`; `breakEvenOfKind` also computes adjusted CSP/long break-evens (executed: AAPL1 60P @1.50 → 58.5); `StrikeLadder.singleGeometry` has no `adjusted` guard and `PositionCard` renders it unconditionally, so the footer prints "—" under a ladder that plots the number with profit/loss zones. AAPL1 80C @1.20 after 3-for-2: ladder $81.20 vs $80.80 on the real 150-share deliverable. Blocking-class default. Plain fix: drop the `??`, null for any adjusted kind, no break-even or zones on an adjusted single; test through the row builder.
- [open] 2026-09-24 · head-of-trading · **Adjusted contracts still enter account totals as complete figures (W36 Finding 2, never ticketed; now traced through condors and single legs).** Flat: `totalRisk([{type:"put_spread",adjusted:true,maxRisk:183}])` → `complete:true` (re-executed). Condor: `syncAccounts` 485–486 + 551–554 sum `putSideRisk`/`callSideRisk` with no adjusted check (read). Single leg: adjusted CSP AAPL1 60P @1.50 → collateral $6,000, notional $5,850 summed into `totals.collateral`/`totals.notional` as complete; the card's "Capital tied up" is unguarded. For a 150-share deliverable, both are understated by $3,000 (33%). Plain fix: an adjusted row contributes nothing to risk/collateral/notional and marks each total incomplete with its ticker.
- [open] 2026-09-24 · head-of-trading · **Covered-call scan prices committed cover as free: bounded max loss, green RoR and break-even on an order that would be uncovered** (`8bff517`). `callCover.scanCover` gives an in-use ticker all its held shares; `buildSingle` builds on them. Executed on the owner's TSLA book (100 sh, short 390C): 410C candidate `maxRisk 24500`, RoR 2.04%, BE 245, ifCalled 16500, ranked among genuine covered calls; the ticket's `short_call_uncovered` warning is correct. Changes what a number means on a surface the owner asked for → **founder**. Recommendation: keep the row; withhold or relabel max risk/RoR/BE/if-called "after closing the 390C"; sort in-use rows after free ones.
- [open] 2026-09-24 · head-of-trading · **`openPosition` labels every single leg with the wheel prefix, and falls back to the spreads prefix without one.** `openPosition/index.ts:212`. Executed through `reconstruct`: (a) short call, wheel prefix, zero shares → filed `covered_call, unpaired:false` while the dashboard says Naked call (`classifyOrphanShort:616`); (b) CSP on a spreads-prefix-only account → `spreads · unpaired`; (c) long call bought from the app → strategy `wheel`, retired by 0011. Order-side fix (plain): only short single legs take the wheel prefix, and there is no spreads fallback for singles. History side (a) and any re-filing of stored rows → **investment-analyst**.
- [open] 2026-09-24 · head-of-trading · **The wheel's adjusted share basis never runs: it queries `trade_records.strategy = 'wheel'`, which reconstruction stopped writing for short legs in 0011.** `syncAccounts/index.ts:183`, `_shared/heldShares.ts:39`. Executed: KO put @1.10 assigned at 90, call @0.60 expired, wheel prefix → 0 matching rows → `{basis: 90, source: "broker"}`; reading the two halves gives `{basis: 88.30, collected: 170, source: "adjusted"}`. Honest label, but `alert-rules.md` says the subtraction happens, and the 2026-09-02 needs-owner item (staging wheel prefix) would not fix it. The query change is plain; switching live cards from broker to adjusted basis changes meaning → founder.
- [open] 2026-09-24 · head-of-trading · **Watch rule gaps against `alert-rules.md`** (executed through the real `evaluate()`): (a) `position_oversized` evaluates option legs only, so 400 KO shares at 60% of equity raise nothing, and assigned wheel stock slips past; (b) `short_through_strike` raises CRITICAL on the short leg of a debit vertical at max profit (long SPY 100C / short 110C, spot 115), a false critical email; (c) an adjusted short put is priced by root "AAPL1", so it permanently raises `price_untrusted` "No price data." with a false reason. (a) and (b) are rule changes: code through head-of-trading, staging first. (c): reason text "adjusted contract", plus a doc line.
- [open] 2026-09-24 · head-of-trading · **`docs/trading/alert-rules.md` rules table is broken**: the `earnings_before_expiry` and `position_oversized` rows sit at lines 76–77 after the `price_untrusted` prose, so they render as stray text and the visible table lists 5 of the 7 rules the watch runs. Doc fix; move both rows back into the table at line 20.
- [open] 2026-09-24 · head-of-trading · **A position with no broker `current_price` is marked at its entry price and captioned "priced from the last trade".** `spreadPairing.ts` `buildLegs` lines 50, 77 (`p.current_price || p.avg_entry_price`). Executed: KO 60P @0.90 and 100 KO @58 without `current_price` → P/L exactly $0.00, and `PositionCard` says "estimated — priced from the last trade". Reach unverified (depends on whether Alpaca omits the field). Plain fix: missing is null; withhold P/L when there is neither a mid nor a broker mark.
