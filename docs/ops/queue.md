...
- [open] 2026-09-16 · seo-editor · **`credit-spread-max-loss.md` has no inbound internal links from any other live post.** Checked every post's outbound `/blog/` links in `content/blog/*.md`: eleven posts are live, and none of them link to `/blog/credit-spread-max-loss`, even though the post itself links out to `intrinsic-vs-extrinsic-value-options`, `options-bid-ask-spread`, `options-journal-splits-spreads-into-legs` and `return-on-risk-vs-return-on-capital`. It is an orphan page today. Fix: add one reciprocal "Read next"-style link to `/blog/credit-spread-max-loss` from `options-bid-ask-spread.md` or `intrinsic-vs-extrinsic-value-options.md` (both share its audience and both are already linked from it), matching the pattern the foundations series already uses for forward links (e.g. `option-delta-explained` ↔ `theta-decay-explained`).
- [open] 2026-09-16 · seo-editor · **`implied-volatility-options-explained.md`'s title is 66 characters** — "Implied volatility explained: why a chain reprices before earnings" — over the ~60-char guideline where SERPs start truncating. The retargeting itself is right and already reasoned through in `docs/seo/keywords.md`'s "Reading for row 8" (away from the exact-match phrase Fidelity/OIC/Schwab hold, toward "why premiums move when the stock doesn't"); only the length needs trimming, e.g. "Implied volatility explained: why chains reprice before earnings" (64) or "Implied volatility: why option chains reprice before earnings" (64).
- [open] 2026-09-16 · seo-editor · **`option-strike-price-expiration-premium.md`'s meta description is exactly 160 characters** — the brief's rule is under 160, and this sits right on the boundary where SERP furniture (site name, date) risks pushing it into truncation. Current: "The strike price is where shares change hands, the expiration is the contract's last day, and the premium is its price. What each number fixes, with an example." Trim by 5-10 characters, e.g. drop "with an example" → "...What each number fixes on a real contract." (148).
