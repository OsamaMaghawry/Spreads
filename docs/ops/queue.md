...
Head-of-trading audited the `call_ratio_spread` close path on 2026-09-08 and
returned **STOP**, binding on the release gate. Two of these can lose money on
the live account today, and the first four change what a number means, so they
are escalations rather than same-day staging fixes. Reproduced by execution
against the real quote and order builders; the 219-test suite catches none of
them.

- [open] 2026-09-08 · head-of-trading · **BLOCKING. A ratio's close sends 1×1 to the broker.** `pairRatios` emits longRatio/shortRatio; every whole-position wire speaks only putRatio/callRatio, so a 1×2 is quoted AND ORDERED as 1×1. Alton Live TSLA 352.50/362.50×2: "Close 1 contract" sends buy 1× 362.50 + sell 1× 352.50, reports "filled", and leaves a short 362.50 open ($581 intrinsic). Whole-tab quote −6.72 vs the true +7.005 — the difference is exactly one 362.50 mid. Confined to `call_ratio_spread`: every other shape's whole-wire ratios agree with `spreadLegs`. `src/components/close/useCloseOrder.js:44-51`, `src/components/close/CloseDialog.jsx:117-124`, `supabase/functions/closeSpread/index.ts:116-117`, `supabase/functions/_shared/alpaca.ts:108-110`. ESCALATE.
- [open] 2026-09-08 · head-of-trading · **BLOCKING. The resumed limit price ignores sign, units and leg set.** `lastAttemptDebit` takes Math.max over any non-filled order's limit_price touching any of the symbols, mixing a per-contract price with a net. Both close buttons on the TSLA ratio resume at +9.89 against a market of −7.01/−6.43 and 6.64/7.37; `nextLimit` returns 9.89 unchanged because it already exceeds the ceiling, and the dialog still promises "never bids above the ask + $0.05". Worst case pays $989 where the correct action RECEIVES $643–701 — a $1,690 swing. **Not confined to the ratio**: a debit vertical with a stale $3.00 on record pays $300 where the market pays $780. `supabase/functions/spreadQuote/index.ts:9-25`, `src/components/close/CloseDialog.jsx:188-196`. ESCALATE.
- [open] 2026-09-08 · head-of-trading · **BLOCKING. The whole-position tab shows +$1,053 on a −$319.50 position** — wrong by $1,372.50. `(netCredit - plDebit) * 100` is the right formula fed the phantom 1×1 quote above, so it resolves when that does. Two aggravating factors in the same block: no `isCredit` branch, so it prints "Mid debit to close −$6.72" where `LegsQuoteSummary` would say "Mid credit"; and `unit` resolves to "contract" when one unit here is three contracts. `src/components/close/CloseDialog.jsx:184, 344-345, 185`. ESCALATE.
- [open] 2026-09-08 · head-of-trading · **BLOCKING. MAX RISK $0.00 on a covered ratio is a default where null belongs.** The options row loses −$2,369 with TSLA at 400 and −$7,369 at 450, above a break-even the same card prints as $376.31 — it names the point past which it loses and then says it cannot lose. The comment refuses to net the shares in (rightly) and then prints a figure that is only true if you do. Being finite, 0 sums into `totalRisk` with `complete: true`, so the account total omits an unbounded exposure while claiming completeness — precisely the class the withhold-rather-than-default rule exists to stop. `supabase/functions/syncAccounts/index.ts:343-345`, `supabase/functions/_shared/positionKinds.ts:382-394`. ESCALATE.
- [open] 2026-09-08 · head-of-trading · **HIGH. "Set my price" cannot express a credit, on any structure.** `PriceControl.set` clamps to 0.01, chips filter `value > 0`, the seed guard is `seed > 0`, `manualReady` is `> 0`, and `replacePrice` throws on `<= 0`. Executed on a debit vertical at mid −8.00: no seed, no chips, `set(−8)` returns 0.01 — so a manual close of any credit-to-close structure is either impossible or submitted with the opposite sign, a $1,600 swing per contract. `src/components/common/PriceControl.jsx:51,58`, `src/components/close/CloseDialog.jsx:204,215`, `src/components/close/useCloseOrder.js:191`. ESCALATE.
- [open] 2026-09-08 · head-of-trading · Leg row prints the broker's last trade as "Current" and computes Unrlzd P/L from it, beside a live NBBO on the same row that contradicts it: the long 352.50C shows $638 at 19.95 next to 20.23/20.66, where the mid gives $687.50 — while the card above it uses mids. `src/components/dashboard/LegRows.jsx:41`.
- [open] 2026-09-08 · head-of-trading · Break-even renders "— – $376.31" on a credit ratio. The null lower break-even is CORRECT — the position is profitable to −∞ — but the em dash means "withheld" everywhere else on this card, so it reads as unknown risk below where there is none. StrikeLadder gets it right. Needs a distinct render, "Below $376.31", not a range. `src/components/dashboard/PositionCard.jsx:21-22`, `src/components/dashboard/SpreadTable.jsx:225-228`.
- [open] 2026-09-08 · head-of-trading · A debit vertical's card prints "NET CREDIT −$300.00" in emerald. `src/components/dashboard/PositionCard.jsx:72`.
- [open] 2026-09-08 · head-of-trading · The legs-tab close button says "(limit)" for both walk and manual, so the screen does not say whether the price will be walked; the whole-position label distinguishes them. `src/components/close/CloseDialog.jsx:457,466`.
- [open] 2026-09-08 · head-of-trading · duty-engineer · `spreadLegs.test.js` has no `call_ratio_spread` case — on the one function that gets the ratio right — and `callCover.test.ts` / `crypto.test.ts` are absent from the `npm test` script, so CI never runs the tests for `ratioCovered`, which gates the maxRisk figure above. `package.json` test script, `src/lib/spreadLegs.test.js`.

Not examined by that audit, and silence there is not clearance: `positionWatch`/`watchRules` against a ratio shape; `PayoffChart`/`TickerPanel`/`tickerBook` (shipped in `fbd520e`, read not at all); `tradeReconstruction` for a closed ratio (investment-analyst); the tax framing of a ratio close (tax-accountant); `manageOrder`'s replace path beyond its `> 0` guard.
