...
seo-editor's weekly review of the 15 live posts, 2026-09-23. No orphans: every
post is linked from at least one other. No `meta_description` is empty or
duplicated. Read from results pages only; Search Console is not connected, so
nothing here is a click or impression figure. Post edits and `docs/seo/keywords.md`
edits go to the daily pipeline. None of these changes a slug.

- [open] 2026-09-23 · seo-editor · **`implied-volatility-options-explained`: title is 66 characters and truncates in results.** Propose `Implied volatility explained: why the whole chain reprices` (58). It keeps the target phrase at the front and states the angle the row 8 reading found unclaimed (a level shift that moves every strike together). The earnings intent stays in the existing H2 "Why does a whole chain reprice before earnings?". That results page is held by "how to avoid IV crush" advice pages, so it is not worth spending title characters on. · `content/blog/implied-volatility-options-explained.md` title: `Implied volatility explained: why a chain reprices before earnings` (66) → `Implied volatility explained: why the whole chain reprices` (58); `docs/seo/keywords.md` row 8 reading.
- [open] 2026-09-23 · seo-editor · **`option-strike-price-expiration-premium`: meta_description is exactly 160 characters, right at the cutoff.** Trim one word so the target phrase "strike price" stays in. · `content/blog/option-strike-price-expiration-premium.md` meta_description: `…What each number fixes, with an example.` (160) → `The strike price is where shares change hands, the expiration is the contract's last day, and the premium is its price. What each fixes, with an example.` (153).
- [open] 2026-09-23 · seo-editor · **`options-journal-splits-spreads-into-legs` never links its category hub.** `content/PLAN.md` says every post links to its category hub. This post links only to `return-on-risk-vs-return-on-capital`. Add one in-body link to `/blog/managing`, for example on "the book" in the closing paragraph before the not-advice line. · `content/blog/options-journal-splits-spreads-into-legs.md` body links: `/blog/return-on-risk-vs-return-on-capital` only → add `/blog/managing`.
- [open] 2026-09-23 · seo-editor · **`return-on-risk-vs-return-on-capital` never links its category hub.** It is the only live post in `measuring`, so nothing in a body links `/blog/measuring` at all. The hub has only the template breadcrumb. Add one in-body link to `/blog/measuring`, for example in "The honest limit of the metric". · `content/blog/return-on-risk-vs-return-on-capital.md` body links: `/blog/options-journal-splits-spreads-into-legs` only → add `/blog/measuring`.
- [open] 2026-09-23 · seo-editor · **`credit-spread-max-loss`: rename one H2 to the question traders actually search.** The M48 reading marks "why is my credit spread showing a loss before expiration" as winnable, because Quora ranks for it. The section under this H2 answers that exact question: a mark that can sit past the credit while expiration has decided nothing. Body unchanged. · `content/blog/credit-spread-max-loss.md` H2 `Before expiration the cap is a boundary, not your current profit and loss` → `Why is my credit spread showing a loss before expiration?`; `docs/seo/keywords.md` M48 reading.
- [open] 2026-09-23 · seo-editor · **`docs/seo/keywords.md` has no row for two live posts.** These are `options-journal-splits-spreads-into-legs` (managing · 50) and `return-on-risk-vs-return-on-capital` (measuring · 59). Add them as off-syllabus rows keyed like `M48`, with the fields below, so the daily run has a target for any revision. **M50** query: `why broker shows spread as two legs`. Results read today: IBKR Campus, TradeStation, TradeZero, protraderdashboard, Wikipedia "Spread trade", and TradingView script pages filling the tail. Winnable. **R59** query: `return on risk vs return on capital credit spread`. Head term: `return on risk`. Today the unqualified phrase goes to bank RAROC/RORAC pages, fixed-income attribution and USPTO patents. Only one options page surfaced: a theoptionpremium credit-spread calculator. So the row must always qualify with "credit spread". Winnable. · `docs/seo/keywords.md` table (rows 1–30 + M48 as of 2026-09-23) has no row for either slug.
- [open] 2026-09-23 · seo-editor · **`docs/seo/keywords.md` rows 1–4 have "Competing today" cells that today's results pages contradict.** These rows also have no reading section, unlike rows 6–12. Update the cells to match what surfaced today. Row 1 `what is an options contract`: Option Alpha (twice), Merrill Edge, Straits Financial, InvestingAnswers. Half the page is contract law (Cornell LII, FindLaw, Wikipedia "Option contract", a Louisiana statute), so always use the plural "options" and never the singular "option contract". Row 2 `call vs put option explained`: SmartAsset, Chase, SoFi, Motley Fool, moomoo, Vanguard, TradingView chart ideas in the tail. Row 3 `option strike price expiration premium`: Fidelity (twice, "selecting" a strike and date), CME, Schwab, OptionsPlay, quantstrategy.io, oboe, TradingView chart ideas. The intent leans to choosing, not defining. Row 4 `intrinsic vs extrinsic value options`: SmartAsset, Angel One, TradingBlock, CapMint, Option Samurai, IncomeShares, Nasdaq, TradingView chart ideas. · `docs/seo/keywords.md` rows 1–4 cells currently name `Investopedia` (all four), `NerdWallet` (row 2) and `tastylive` (row 4). None of those surfaced on 2026-09-23.
