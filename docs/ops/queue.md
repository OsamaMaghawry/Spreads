...
- [open] 2026-09-17 · head-of-trading · **The entry path treats shares already backing a short call as free cover.** `_shared/heldShares.ts:51` returns the broker's raw share count; `optionScan.buildSingle`'s `covered_call` branch and `openPosition`'s `coverWarning` (`_shared/orderWarnings.ts`) both size/warn off that raw count instead of `allocateCallCover` (`_shared/callCover.ts`), the one allocator the dashboard and watch already agree on. Executed: account holds 100 TSLA, already short 1× TSLA 261016C450 — `openPosition` raises no warning and `optionScan` prices a *second* short TSLA call as `ok=true, maxRisk=$29,400` when the shares are already spoken for; after the fill, `allocateCallCover` on both shorts reports the second one `uncovered:0` → naked. TSLA at 420, short 460C for $6: at 600 the "covered" contract has lost $13,400 and rising, against a $29,400 "worst case" on the ticket. Blocking (changes what a number means) — hand-off systems-engineer: route both call sites through `allocateCallCover` and have `coverWarning` take an allocation, not a share count. Full repro in `docs/trading-audit/2026-W38.md` Finding 1.
- [open] 2026-09-17 · head-of-trading · **A short stock lot is priced with a finite max loss and the wrong-signed stress P/L.** `_shared/positionKinds.ts` `stressPL`/`stressLossOfKind`/`riskOfKind`, case `SHARES`, `Math.abs` the share qty — a short lot scores identically to a long lot. Executed: short 100 TSLA @$400 basis, spot $420 → `maxRisk=$40,000` (finite, on an upside-unbounded position), `stressPL` at +15% reports +$6,300 (should be −$6,300). Netted with a short put (the post-assignment "reverse wheel" shape, which the product already anticipates elsewhere): reports `{risk:9800,complete:true}` against a true $5,500 — 78% overstated with no floor indicator; flip the second leg to a short call and it understates instead. Reaches `totals.notional` as a complete number labelled "Max loss (stock to 0)". Blocking — hand-off systems-engineer: carry the sign through `stressPL`/`stressLossOfKind`; `riskOfKind(SHARES)` must return `null` (unbounded) for negative `shareQty`, same posture as `NAKED_CALL`. Full repro in `docs/trading-audit/2026-W38.md` Finding 2.
- [open] 2026-09-17 · head-of-trading · `syncAccounts/index.ts:297,472` computes `moneyness` off `stockPrice > 0` alone and never reads `spotTrusted` (the same field it puts on the row) before painting ITM/OTM; neither `PositionCard.jsx:132` nor `SpreadTable.jsx:181,191` reads it either — `OptionChain.jsx:259` does, and is what the dashboard should match. Executed on the JPM shape from `marketPrice.ts`'s own header (trade $354.33 vs quote mid $363.54, `trusted:false`): `syncAccounts` still renders `moneyness:"ITM"` with nothing on screen saying the spot is untrusted. Worst branch is the stale `dailyBar` fallback painting a confident green chip on a halted/thin name. Hand-off systems-engineer: withhold the chip (slate, as `moneynessCell` already does for null) when `!spotTrusted`. Full repro in `docs/trading-audit/2026-W38.md` Finding 3.
- [open] 2026-09-17 · head-of-trading · `_shared/callCover.ts:78-117`'s `longPool` covers a short call with the *first* eligible long in array order, not the nearest — same "first found, not nearest" defect as the still-open `pairSide` ticket below, reproduced inside the module written 8 Sep specifically to be the one source of truth. Executed end-to-end through `pairSpreads`: short NVDA 261016C150 with two eligible longs (different-expiry NVDA 270115C160 and C250) — broker order `[short,250L,160L]` covers by 250 (`maxRisk=$9,500`); order `[short,160L,250L]` covers by 160 (`maxRisk=$500`). Same four legs, same account, no trade in between, $9,000 swing in the Detailed table and `totals.notional` from Alpaca's undocumented list order. **Fix together with the `pairSide` ticket below — same non-deterministic-ordering defect, two files.** Hand-off systems-engineer. Full repro in `docs/trading-audit/2026-W38.md` Finding 4.
- [open] 2026-09-17 · head-of-trading · A poor-man's covered call (long LEAP call + short near-dated call, no shares) is priced correctly (`maxRisk=0`, the long's premium sits on its own row) but labelled and captioned as a real covered call: `_shared/positionKinds.ts` `breakEvenOfKind(COVERED_CALL)` computes `shareBasis − credit` with `shareBasis=0` when cover came from `coverLongs` rather than a share lot. Executed: long TSLA 270115C250 @$180, short TSLA 261016C400 @$12, no shares → card shows badge "CC", `Break-Even −$12.00`, "Max loss (stock to 0)", "Broker basis" tooltip — none of which describe this position (real break-even ≈ $418). Hand-off systems-engineer: `breakEvenOfKind(COVERED_CALL)` returns `null` on this branch, plus a label that isn't "stock to 0". Full repro in `docs/trading-audit/2026-W38.md` Finding 5.
- [open] 2026-09-17 · head-of-trading · An unbalanced iron condor (e.g. 2:1 put:call ratio, from a hand-built or externally-opened multi-leg order) closes 1:1. `syncAccounts/index.ts:481-482` stamps `longRatio:1/shortRatio:1` on every non-ratio row, and `src/lib/spreadLegs.js:112-123` reads `ratio: spread.shortRatio || putRatio` — the stamp shadows the real `putRatio`. Executed: a `putRatio:2, callRatio:1` condor sends all four legs at ratio 1 through the close ticket instead of the puts at ratio 2, leaving one put spread per unit open after a ticket that priced and confirmed the whole structure closed. Up to $5,000/unit residual risk on a 50-wide SPX condor believed flat. Not producible by the scanner — rare, but exactly a ticket-vs-broker mismatch. Hand-off systems-engineer: stop `spreadLegs` falling back through `shortRatio` for a condor, or stop `syncAccounts` stamping ratios on non-ratio rows. Full repro in `docs/trading-audit/2026-W38.md` Finding 6.
- [open] 2026-09-17 · head-of-trading · `positionWatch/index.ts:46,277` filters to `parseOCCSymbol(p.symbol)` before any rule runs, so `_shared/watchRules.ts`'s `evaluate` never sees a share position — only as a cover count, via `sharesByTicker`, which itself drops short shares (`if (!(qty > 0)) continue`). Two consequences, both executed/read-derived: `position_oversized` (documented as firing on "one position's market value" via the broker's own figure, specifically to hold when our pricing is suspect) cannot fire on a stock holding at all; and a short stock lot (the unbounded-upside shape from the "short stock stress sign" ticket above) is watched by nothing, contradicting `alert-rules.md`'s own opening claim that a naked short is the one thing this watch exists to catch. Hand-off systems-engineer for the code; head-of-trading owns the `alert-rules.md` rewrite once the direction is settled. Full repro in `docs/trading-audit/2026-W38.md` Finding 7.
- [open] 2026-09-17 · head-of-trading · The session watch takes the *live-ladder* branch on a market holiday or post-half-day afternoon, the opposite of what `_shared/watchRules.ts`'s own comment says it does. Executed: `sessionPhase` returns `"open"` on Thanksgiving and an hour past a 1pm ET half-day close, so `positionWatch/index.ts:289-292`'s `mode==="daily" || !live` is false and it reads the live 30-minute ladder — which is untrusted on a day nothing is trading, so every ticker raises `price_untrusted` and `short_through_strike`/`short_near_strike` never get reached. ~9 full holidays + ~3 half-days/year; the holiday case is benign (nothing moves), the half-day afternoon is not. `reconcile`'s `PRICE_DEPENDENT` guard still correctly keeps existing criticals open throughout. Hand-off systems-engineer: a holiday/half-day calendar, or fall back to the close price when the live ladder is untrusted rather than suppressing every price-dependent rule. Full repro in `docs/trading-audit/2026-W38.md` Finding 8.
- [open] 2026-09-17 · head-of-trading · A partly-covered call ratio (e.g. 1 long / 3 short with 100 covering shares) is tagged `structure:"stock_repair"` by `_shared/spreadPairing.ts`'s `tagStructures:725` and the card prints "—" for its unbounded excess short instead of the "Unlimited" that `naked_call` shows for the identical exposure elsewhere. The pairing already computes the honest fields — `ratioCovered`, `coverShares`, `coverLongs`, `coveredBy`, `excessShorts` — confirmed by grep to appear nowhere under `src/`: computed, never rendered. Not misleading in the places that matter (account total is `null`/floored, watch raises `naked_short_call` on the raw leg) but the card itself is falsely reassuring. Hand-off systems-engineer to wire the fields into the card; wording is head-of-trading's. Full repro in `docs/trading-audit/2026-W38.md` Finding 9.
- [open] 2026-09-17 · head-of-trading · `syncAccounts/index.ts:304` substitutes a computed break-even (`?? (isCall ? strike+entryPrice : strike-entryPrice)`) exactly when `_shared/positionKinds.ts`'s `breakEvenOfKind(SHORT_CALL_UNJUDGED)` deliberately returns `null` — a corporate action changed the deliverable, so per-share strike-plus-premium arithmetic is meaningless. Executed: short 1× AAPL1 261016C150 @$3.25 → `positionKinds` says `null`, `syncAccounts` renders `153.25` on the payload. Currently harmless — both `PositionCard` and `SpreadTable` branch on `s.adjusted` first and print "—" regardless — but it's a fabricated number one un-guarded consumer away, in a file whose neighboring comments are all about not doing this. Low severity given today's UI guard. Hand-off duty-engineer: one-line fix, `s.breakEven !== undefined ? s.breakEven : …` (or drop the fallback for withholding kinds). Full repro in `docs/trading-audit/2026-W38.md` Finding 10.
- [open] 2026-09-17 · head-of-trading · The moneyness chip is computed at sync time and never recomputed by the live price stream: `AccountSection.jsx:99`'s overlay rewrites `stockPrice` and sets `spotTrusted:true` for the `StrikeLadder` marker (`StrikeLadder.jsx:161`) but not the `moneyness` field the chip reads (`PositionCard.jsx:132`). Normal window is seconds (`useLiveSync`'s idle re-sync), widening to 5-60s under rate-limit backoff and unbounded while a refresh is failing — precisely the state `StaleDataNotice` displays while the stream keeps moving the marker under a frozen chip. Read-derived. Hand-off systems-engineer: recompute moneyness client-side from the same `stockPrice` the ladder uses, or blank the chip when `spotSource === "stream"`. Full detail in `docs/trading-audit/2026-W38.md` Finding 11.
- [open] 2026-09-17 · head-of-trading · `docs/trading/alert-rules.md` lines 76-77 (`earnings_before_expiry`, `position_oversized`) sit stranded 60 lines below the rules table they belong to, after an intervening prose block — they won't read as part of it. Head-of-trading's own file to fix; will correct in the same change that resolves the `position_oversized`-blind-to-shares and holiday-branch tickets above, staging-first. Full detail in `docs/trading-audit/2026-W38.md` Finding 12.
- [open] 2026-09-17 · head-of-trading · **Last week's adjusted-contract total-risk finding (2026-W36 #2) was never given a queue ticket — raising it now, still unfixed.** `syncAccounts/index.ts:383` computes `maxRisk` for every flat 2-leg spread unconditionally; `isAdjusted` (line 449) never gates it; `totalRisk` (`_shared/positionKinds.ts`) has no knowledge of `.adjusted` at all. Executed: `totalRisk([{type:"put_spread", adjusted:true, maxRisk:183}]) -> {risk:183, complete:true, undefinedRisk:[]}` — a fabricated max risk on an adjusted contract's width reaches `account.totals.risk`/`riskPct` as a confident, complete number, no floor indicator. Per-row display already withholds correctly (`SpreadTable.jsx`); only the total is wrong. Hand-off systems-engineer: null out `maxRisk` on an `adjusted` flat-spread row before `totalRisk` sees it, or teach the reducer to treat `.adjusted` as undefined risk. Full detail in `docs/trading-audit/2026-W38.md` Finding 14 (originally `2026-W36.md` Finding 2).
