...
// Renders nothing for ordinary users; the edge function enforces the same check
// server-side, so hiding this is presentation, not the access control.
export default function AdminMaintenance() {
  ...
  const run = async () => {
    ...
    setError("");
    setResult(null);
    const res = await invokeFunction("migrateCredentials");
    setRunning(false);
    ...
    <div className="bg-white border border-slate-200 rounded-xl px-5 py-4">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <div className="text-sm font-medium text-slate-900">Encrypt legacy credentials</div>
          <div className="text-xs text-slate-500 mt-0.5">
            Encrypts any credentials still stored in plaintext, for all users. Safe to run repeatedly.
          </div>
        </div>
        <button
          onClick={run}
