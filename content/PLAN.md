...
| 2026-09-18 | `gamma-options-meaning` | foundations · 9 | Investopedia shape. One original SVG (`gamma-across-moneyness`, three bars on a 0–0.08 scale showing gamma peaking at the money rather than climbing like delta) plus two tables — the worked example reuses the same hypothetical 50-strike 30-day call and the same three stock prices and deltas as `option-delta-explained`, and the second table tracks at-the-money against far-out-of-the-money gamma at 30, 7 and 1 days. Reviewed — desk-editor (re-derived every gamma figure against a Black-Scholes fit at the series' 40% IV and confirmed all eight; two blocking findings fixed: a "smallest room left to change" line had the middle and outer rows backwards, and a closing claim that gamma explains why theta/IV readings are snapshots was wrong — that is a delta-only property; also added the call/put gamma-parity caveat, linked `theta` on repeat mention, and added `published_at`), seo-editor (studied row 9 fresh — "gamma options meaning" reads as the same intent as "what is gamma in options" and bare "gamma" drifts to a dictionary/disambiguation page, so the title now carries "gamma in options" while the slug, as the target query, is unchanged; meta description picked up the near-expiration angle; the one non-question H2 in the series reworded to a question with an answer-first opening; corrected row 9's head-term and competing-domains cells in `docs/seo/keywords.md` against today's read — Investopedia does not actually surface on any gamma phrasing), compliance-gate (no findings, ship — SVG source read directly, no broker name, no feature claim, no performance figures). `npm run content:check` clean, whole-repo check also clean. Also updated `implied-volatility-options-explained`'s Bottom Line with the "Read next" forward link to this post |
