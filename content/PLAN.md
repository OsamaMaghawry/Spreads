...
| 2026-09-24 | `options-buying-power-requirement` | foundations · 13 | draft, Investopedia shape. Two original SVGs (`buying-power-put-vs-spread`, `buying-power-hold-flat`) plus one table. Worked example on one hypothetical $10,000 margin account, short the same 50 put two ways: cash-secured ($1.40 credit, $5,000 reserved, net BPR $4,860) and as a 50/48 put credit spread ($0.60 credit, $200 reserved, net BPR $140). Covers buying power vs cash balance, working-order reservations, spread-margin conditions (same account, long leg expiring no earlier), covered call collateral in brief, naked requirements as broker/exchange-set without universal figures, and what releases the hold (close, expiry, assignment — not the mark). No product claims made. Links to `what-happens-options-expiration`, `what-is-an-options-contract`, `credit-spread-max-loss`, `option-assignment-what-happens`, `/blog/foundations` and `/blog/income`; `what-happens-options-expiration`'s closing line now links forward to it. `npm run content:check` clean. Unreviewed — awaiting desk-editor, then seo-editor, then compliance-gate |
