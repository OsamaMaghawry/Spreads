...
| 2026-09-06 | `what-is-an-options-contract` | foundations · 1 | reviewed and pushed to `claude/compassionate-edison-zozunc`, NOT on main. content-engine, seo-editor, desk-editor and compliance-gate all ran this session with a clean `content:check`; ship verdict from every gate. `content-merge.yml`'s path allowlist doesn't include `landing/public/assets/blog/`, so the auto-merge step skips on every push carrying this post's required SVGs — the same reason three earlier attempts at this exact post (3, 4 and 5 Sep, branches `jbvqqh`, `3j5z5w`, `vy90xb`) are also still stranded, unmerged, never live. Needs a person to merge one of these branches (this one is the most recently reviewed) or the workflow's regex fixed. |
