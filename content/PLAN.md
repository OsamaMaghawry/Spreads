...
| 2026-09-18 | `gamma-options-meaning` | foundations · 9 | draft, Investopedia shape. One original SVG (`gamma-across-moneyness`, three bars on a 0–0.08 scale showing gamma peaking at the money rather than climbing like delta) plus two tables — the worked example reuses the same hypothetical 50-strike 30-day call and the same three stock prices and deltas as `option-delta-explained`, and the second table tracks at-the-money against far-out-of-the-money gamma at 30, 7 and 1 days. Prose 1,031 words, 1,655 total, 8 `##` sections, 4 lists, 3 visuals. `npm run content:check` clean. Also updated `implied-volatility-options-explained`'s Bottom Line with the "Read next" forward link to this post. **Unreviewed — desk-editor, then seo-editor, then compliance-gate all outstanding** |
