...
| 2026-09-26 | `covered-call-explained` | income · 15 | First income post, opens the series `how-to-read-an-option-chain` hands off to. Investopedia shape, one original SVG (`covered-call-payoff`, covered call vs shares alone at expiration: premium band, break-even 46.58, cap above the 52 strike, upside given up) plus one table (close below vs above the strike). Worked example: hypothetical 100 XYZ shares bought at $48.00, stock at $50.00, one 30-day 52 call sold at the 1.42 bid from the `how-to-read-an-option-chain` quote ($142 premium, break-even $46.58, most above cost $5.42/share), closes at $51.00, $56.00 and $45.00. Covers covered vs naked, the three ways the call leaves, early assignment before ex-dividend, effective break-even vs reported cost basis (tax left to a professional), and ends on the account the day after assignment. No product claims, no broker named. Reviewed — desk-editor (two blocking findings fixed: the "shares only if below the strike" Key Takeaway contradicted the post's own early-assignment and buy-back cases, reworded to the expiration case only; the assignment-notice timing was stated as certain against `option-assignment-what-happens`'s own "usually", softened to "usually...before Monday's open, though it can arrive later" — plus cautions applied: "if the buyer/holder exercises" corrected to "if assigned" and "call buyer" to "call holder" throughout since assignment is randomised rather than tied to the original counterparty, "every dollar the shares rise" scoped to "at expiration", the covered-shares-cannot-be-sold claim qualified to "unless the account is approved to hold naked calls", the SVG's aria-label corrected from "value" to "gain or loss... against the $48 cost", and one sentence added naming the $53.42 break-even-plus-strike point where the SVG's "given up" wedge actually starts. Arithmetic, the SVG geometry and the reused option-chain quote all re-derived and confirmed correct), seo-editor (row 15 query-space study added to `docs/seo/keywords.md` — the bare "covered call" head term is bank/broker-education hard, but "what happens to the account after assignment / shares called away" is a text-coverage gap this post's angle already targets; title shortened from 67 to 59 characters to keep "assignment" inside the render budget, meta description rewritten to state the answers rather than list the sections, one FAQ bullet added for "called away", and reciprocal "Read next" links added from `how-to-read-an-option-chain` and `options-buying-power-requirement` since the automatic same-category nav can't link across foundations→income; `docs/seo/keyword-map.md` item 19 updated to point at the new reading), compliance-gate (no findings, ship — checked the post, the two link-only backlink edits and the SVG's diagram text/aria-label against every compliance rule and the claims-discipline list; no advice, no performance figures, no guarantees, no broker named, tax paragraph reads as a deferral rather than a rule, disclaimer present). `npm run content:check` clean across all three touched files. Links `/blog/income`, `how-to-read-an-option-chain`, `options-buying-power-requirement`, `what-is-an-options-contract`, `early-exercise-options`, `option-assignment-what-happens`; also linked forward from `how-to-read-an-option-chain` and `options-buying-power-requirement` |
