...
| 2026-09-19 | `gamma-options-meaning` | foundations · 9 | Investopedia shape, two original SVGs (`gamma-is-the-slope-of-delta`, tangents on the delta curve at $44 vs the $50 strike; `gamma-by-days-left`, the same curve at 30 vs 7 DTE) plus three tables, worked example walking one hypothetical 50-strike call's delta across four nearby stock prices and across four days-to-expiration points, all figures a Black-Scholes fit consistent with the delta, theta and IV posts' own numbers. Reviewed — desk-editor (three blocking findings fixed: the Key Takeaways and "why gamma rises" section stated unqualified that gamma grows near expiration, false for an out-of-the-money strike where it peaks mid-life and fades — scoped to at-the-money with the OTM case added, mirroring theta's own caveat; the first diagram's caption/aria-label claimed the $44 tangent was "several times" flatter than the $50 one when the true ratio is about 1.5x — reworded to "noticeably flatter" / "about half again as steep" in the SVG and its labels; a sentence said negative gamma "picks up exposure on the side the stock is already moving" when it is the opposite — corrected to "moving away from", matching the post's own table two lines below; several cautions also applied: call-vs-put delta bounds, the spread-gamma FAQ qualified to "near the short strike", the 0.52-vs-0.50 FAQ sharpened, `published_at` added), seo-editor (studied the query space fresh and corrected two inherited errors in the pre-existing keyword row — Investopedia does not compete here and unqualified "gamma" collides with gamma.app/gamma-correction the way "theta" collided with crypto, so the head term becomes "option gamma"; kept the title as-is rather than forcing the literal "gamma options meaning" phrase into a SERP dominated by broker hubs with no forum-shaped coverage; retargeted row 9 to the winnable "when does gamma matter" / "why does gamma increase near expiration" long tail already answered by the post's own H2s; reworded two headings to match; strengthened the inbound anchor text on the forward link from `option-delta-explained`), compliance-gate (no findings, ship). `npm run content:check` clean across all 16 scanned files. Also updated `option-delta-explained`'s closing paragraph with a "Read next" link forward to this post |
