...
| 2026-09-15 | `gamma-options-meaning` | foundations · 9 | Investopedia shape, one original SVG (`gamma-is-deltas-slope`, delta plotted as an S-curve against stock price with the strike marked) plus two tables, worked example carrying the running hypothetical 50-strike/30-day chain one dollar in each direction ($49/$50/$51) and pricing the $50→$51 move against the IV post's own $2.29 premium. Reviewed — desk-editor (three blocking findings fixed: the worked example stated a $0.56 gain against $2.29/$2.84 premiums that only support $0.55, now carries enough precision — $2.2862/$2.8434, $0.5572 — that the arithmetic actually follows; "the hump the diagram above draws" corrected, since the SVG plots a monotonic delta curve, not a hump; the "second order" takeaway now names delta, not the chain's raw inputs, as what gamma describes the shifting of; two cautions also applied — a one-line reconciliation of the 0.52-vs-0.50 at-the-money delta rounding against `option-delta-explained`, and a ceiling added to the "compounds on a sold option" bullet so it doesn't read as unbounded; desk-editor independently recomputed every figure, including the SVG's plotted curve to sub-pixel accuracy, against a Black-Scholes fit and confirmed the whole series' hypothetical chain is internally consistent), seo-editor (row 9 query-space study added to `docs/seo/keywords.md`, correcting head term to "option gamma" and the competing-domains cell — Investopedia does not surface for any of the ten phrasings read; two headings reworded to searched phrasing; added the "long gamma / short gamma" terms where the post already covered the concept; added the reciprocal inbound link from `option-delta-explained`'s FAQ, which asked the exact question this post answers), compliance-gate (no findings, ship). `npm run content:check` clean |
