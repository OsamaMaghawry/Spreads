...
| 2026-09-07 | `what-is-an-options-contract` | foundations · 1 | live. Syllabus #1, targets "what is an options contract". Full review chain run: seo-editor (title, meta, lede, three headings, one inbound link from `return-on-risk-vs-return-on-capital`), desk-editor (fixed a Saturday expiration date in both diagrams, a mark-vs-closing-cost error, an unqualified physical-settlement claim, and several smaller mechanics precision fixes), compliance-gate (ship, no findings). `content:check` clean |
