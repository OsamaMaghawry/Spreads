...
None of the six is something you set; you choose which listed series to trade
— this underlying, this strike, this expiration — not the terms written into
it.

The seventh number is the premium — the only term either side negotiates. It
is what you pay or receive, and what the quote on your screen shows.
...
with the same six terms is identical to every other, your contract isn't a
private arrangement with whoever filled it. The clearing house stands between
you both, so you never have to find that person again — you close by trading
the opposite way with anyone at all, and the two offsetting positions cancel
...
$215 before fees. That factor of a hundred is a contract term, not a display
convention — and it is the arithmetic that catches people first, usually in
the direction of an order a hundred times larger than intended.

The same factor applies further down. A 105 strike is not $105 of anything; if
exercised, 100 shares change hands at $105 each, so the transaction behind the
contract is $10,500. The premium is the small number, the notional is the
large one, and the two are worth keeping straight, because size later on is
measured against the second.

The deliverable is also the term most likely to be quietly amended. After a
stock split, merger, spin-off or special dividend, the clearing house adjusts
existing contracts, and an adjusted one can deliver something other than 100
ordinary shares — a different share count, a cash component, shares in a
company that didn't exist last month. Adjusted contracts are uncommon and
carry a marker in the symbol; "times 100 shares" is a default, not a law.
...
are several contracts traded as one order — a spread, a condor — and your
broker's fill feed reports them as separate executions that something
downstream has to reassemble. Done by resemblance rather than by reading the
order the legs came from,
